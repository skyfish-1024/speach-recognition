...
var printTextarea = (function () {
    return function (text) {
        ...
        } else if (text.includes("full_default")) {
            textArray = [];
        } else if (text.includes("whisper_print_timings")) {
            sendMessage({
                evt: "speech-recognition-succeed",
                text: textArray,
            });
        } else {
            // console.log(text);
...
async function clearCache() {
    if (
        ...
        indexedDBValid.deleteDatabase(dbName);
        location.reload();
    }
}
...
function loadRemote(url, dst, size_mb, cbProgress, cbReady, cbCancel, cbPrint) {
    ...
    // check if the data is already in the IndexedDB
    var rq = indexedDBValid.open(dbName, dbVersion);
